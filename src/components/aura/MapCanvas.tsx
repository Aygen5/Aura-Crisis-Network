...
import { disasterMeta, type AuraEvent } from "@/lib/aura-data";
import { TURKEY_PATHS, NEIGHBOR_PATHS, CITIES, SEA_LABELS } from "@/lib/geo-turkey";
import { cn } from "@/lib/utils";
import { toneBg, toneText } from "./primitives";

type Props = {
  events: AuraEvent[];
  active?: Record<string, boolean>;
  selectedId?: string | null;
  onSelect?: (e: AuraEvent) => void;
  className?: string;
  compact?: boolean;
};
...
export function MapCanvas({
  ...
  const visible = events.filter((e) => {
    if (e.type === "report") return isOn("report");
    if (["earthquake", "flood", "wildfire"].includes(e.type)) return isOn(e.type);
    return true;
  });
  ...
  return (
    <div className={cn("relative overflow-hidden bg-background", className)}>
      ...
      {/* subtle scan sweep — realtime feel, not decoration */}
          ...
          >
            <span className="relative flex items-center justify-center">
              {e.status === "Active" && (
                <span
                  className={cn(
                    "absolute h-8 w-8 rounded-full opacity-40 animate-ping-slow",
                    toneBg[tone],
                  )}
                />
              )}
              <span
                className={cn(
                  "relative flex items-center justify-center rounded-full border backdrop-blur-sm transition-all duration-200",
                  compact ? "h-6 w-6" : "h-8 w-8",
                  isSel || hover === e.id ? "scale-115" : "scale-100",
                  `border-current/40 ${toneText[tone]}`,
                )}
              >
                <span className={cn("absolute inset-0 rounded-full opacity-15", toneBg[tone])} />
                <span className={cn("relative", compact ? "h-3 w-3" : "h-4 w-4")}>
            ...
            {hover === e.id && !compact && (
              <span className="glass absolute left-1/2 top-full z-20 mt-2 w-56 -translate-x-1/2 rounded-lg p-3 text-left animate-scale-in">
                <span className="block text-[13px] font-medium text-foreground">{e.title}</span>
                <span className="mt-0.5 block text-[11px] text-muted-foreground">
                  {e.district} · {e.source}
                </span>
                <span className={cn("num mt-2 block text-sm font-semibold", toneText[tone])}>
                  {e.metric}
                </span>
              </span>
            )}
